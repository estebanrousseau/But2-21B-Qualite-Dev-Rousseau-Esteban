...
class Jeu:
	...
	def start(self):
		self._afficherTexte('Tetris', constante.CENTRE_FENETRE, font = 'titre')
		self._afficherTexte('Appuyer sur une touche...', constante.POS)
		self._attente()

	def stop(self):
		self._afficherTexte('Perdu', constante.CENTRE_FENETRE, font='titre')
		self._attente()
		self._quitter()
...
#		print("Position testée valide: x=%s, y=%s" % (x, y))
	...
	def _gererEvenements(self):
		...
		if event == K_p:
			...
			self._afficherTexte('Pause', constante.CENTRE_FENETRE, font='titre')
			self._afficherTexte('Appuyer sur une touche...', constante.POS)
			self._attente()
		elif event == K_LEFT:
	...
	def _dessinerPlateau(self):
		...
		self._afficherTexte('Score: >%s' % self.score, constante.POSITION_SCORE)
		self._afficherTexte('Pièces: %s' % self.pieces, constante.POSITION_PIECES)
		self._afficherTexte('Lignes: %s' % self.lignes, constante.POSITION_LIGNES)
		self._afficherTexte('Tetris: %s' % self.tetris, constante.POSITION_TETRIS)
		self._afficherTexte('Niveau: %s' % self.niveau, constante.POSITION_NIVEAU)
